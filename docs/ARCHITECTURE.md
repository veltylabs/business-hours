...
3. **MCP Self-Registration:** `*Module` implements `mcp.ToolProvider` via `Tools()`.
   Registered by passing `*Module` as a provider to `mcp.NewServer(config, []mcp.ToolProvider{m})` at the composition root.
